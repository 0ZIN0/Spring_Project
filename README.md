# 🖥 Show Me Games

<p align="center">
  <img src="ShowMeGames/src/main/webapp/resources/img/logo/smg_loginLogo.png" width=1000>
</p>

---

# 🛠 Dev
개발 기간 : 2023.07.13 ~ 2023.08.23
<br>
🏃🏻‍♀️ 리드미 지속적으로 작성중

---

## 팀원 소개
<table>
  <tr>
    <th>이로아</th>
    <td rowspan="3" width = 500>우리 처음 만났던 어색했던 그 표정속에<br>서로 말놓기가 어려워 망설였지만</td>
  <tr>
    <td> 
      <img src="./readme_img/roa_profile.jpg" width = 200>
    </td>
  </tr>
  <tr>
    <th>
      PA
    </th>
  </tr>
</table>

<table>
  <tr>
    <th>김찬우</th>
    <td rowspan="3" width = 500>
      음악속에 묻혀 지내온 수많은 나날들이<br>
      이젠 돌아갈 수 없는 아쉬움됐네
    </td>
  <tr>
    <td> <img src="https://github.com/0ZIN0/Spring_Project/assets/112708158/f32337aa-d361-4d1b-8012-323e36dc39ca" width = 200></td>

  </tr>
  <tr>
    <th>
      PL
    </th>
  </tr>
</table>

<table>
  <tr>
    <th>손재훈</th>
    <td rowspan="3" width = 500>
      이제는 우리가 서로 떠나가야할 시간<br>
      아쉬움을 남긴채 돌아서지만
    </td>
  <tr>
    <td> <img src="./readme_img/shj.png" width = 200></td>
  </tr>
  <tr>
    <th>
      PA
    </th>
  </tr>
</table>

<table>
  <tr>
    <th>장영민</th>
    <td rowspan="3" width = 500>
      시간은 우리를 다시 만나게 해 주겠지<br>
      우리 그 때까지 아쉽지만 기다려봐요
    </td>
  <tr>
    <td> <img src="./readme_img/jym_profile.png" width = 200></td>
  </tr>
  <tr>
    <td>PA</td>
  </tr>
</table>

<table>
  <tr>
    <th>배기철</th>
    <td rowspan="3" width = 500>어떻게 해서든 1인분은 하겠습니다.</td>
  <tr>
    <td> <img src="./readme_img/penguin.jpg" width = 200></td>
  </tr>
  <tr>
    <td>문의하기<br>스크롤 버튼<br>관리자-문의하기</td>
  </tr>
</table>

<table>
  <tr>
    <th>홍기태</th>
    <td rowspan="3" width = 500>
      기억속에 묻혀진 작은 노래됐지만<br>
      우리들 맘엔 영원히
    </td>
  <tr>
    <td> <img src="./readme_img/hgt_profile_img.png" width = 200></td>
  </tr>
  <tr>
    <th>
      PA
    </th>
  </tr>
</table>


---

# 🌱 프로젝트 동기
#### 웹의 다양한 기술을 사용할 수 있는 쇼핑몰 사이트를 구상하였습니다. <br> 팀원 모두가 좋아하는 게임을 주제로, 화려한 색감과 애니메이션을 사용하여 한 눈에 띄는 웹을 목표로 하였습니다.
---
# 🎢 기술 스택(Stacks)

<img src="https://img.shields.io/badge/html-orange?style=for-the-badge&logo=html&logoColor=orange"> <img src="https://img.shields.io/badge/css-blue?style=for-the-badge&logo=html&logoColor=white"> 
 <img src="https://img.shields.io/badge/github-181717?style=for-the-badge&logo=github&logoColor=white"> <img src="https://img.shields.io/badge/java-2A6379?style=for-the-badge&logo=java&logoColor=white"> <img src="https://img.shields.io/badge/oracle-F80000?style=for-the-badge&logo=oracle&logoColor=white">  
 <img src="https://img.shields.io/badge/spring-gray?style=for-the-badge&logo=spring&logoColor=green"> <img src="https://img.shields.io/badge/javascript-yellow?style=for-the-badge&logo=javascript&logoColor=gray"> <img src="https://img.shields.io/badge/jquary-white?style=for-the-badge&logo=jquary&logoColor=0080ff"> <img src="https://img.shields.io/badge/aws-orange?style=for-the-badge&logo=aws&logoColor=orange">  
<img src="https://img.shields.io/badge/Adobe Photoshop-31A8FF?style=for-the-badge&logo=Adobe Photoshop&logoColor=white"> <img src="https://img.shields.io/badge/figma-F24E1E?style=for-the-badge&logo=figma&logoColor=white">
<img src="https://img.shields.io/badge/mybatis-black?style=for-the-badge&logo=mybatis&logoColor=black">  
<img src="https://img.shields.io/badge/visual studio code-blue?style=for-the-badge&logo=visualstudiocode&logoColor=black">  
 
---

# 📝 프로젝트 웹 사이트 접속 
### (❌ AWS 비용 청구로 인해 폐쇄한 상태입니다.)

<a href="http://3.39.6.147:8090/smg/"><img src="ShowMeGames/src/main/webapp/resources/img/logo/smg_loginLogo.png" width=300 height=70></a>
<br>

---

## 💻 프로젝트 소개
<table>
<th>
메인 페이지
</th>
<th>
설명
</th>
<tr>
<td>
<img src="readme_img/project_index_page.png" width=150 height=500>
</td>
<td>
<h3>
(📷 사진을 클릭하시면 확대하여 보실 수 있습니다.) 
</h3>
<h3>
구성
</h3>
해상도 1920 x 1080 기반의 풀스크린 웹으로 제작하였습니다.
<br>
헤더와 푸터가 존재하고 게임 이미지와 가격정보를 보여주는 "카드"가 존재합니다.
<br>
카드를 담고있어 넘겨볼 수 있는 "카드 슬라이더"가 존재합니다.
<br>
배경색은 3겹의 레이어로 보다 자연스럽고 화려한 그라디언트로 제작하였습니다.
<br>
<h3>
기능
</h3>
메인페이지에서는 다양한 메뉴로 접속이 가능합니다. 상단 헤더에 공지사항, 고객지원, 로그인이 있고 밑에 다양한 카테고리, 검색 기능, 장바구니가 있습니다.
<br>
카드를 클릭하면 해당하는 게임으로 페이지가 이동됩니다. 카드 슬라이더를 통하여 다양한 게임들을 메인 페이지에서 볼 수 있도록 제작하였습니다.
<h3>
애니메이션
</h3>
1. 스크롤 다운하면 헤더의 밑 부분만 보여주게됩니다. 업하면 원래대로 복구됩니다.
<br>
2.헤더의 버튼에 마우스를 올리면 색이 변합니다. 클릭 할 시 화살표가 회전합니다.
<br>
3. 카드 슬라이더의 버튼을 누르면 자연스럽게 넘어갑니다. 더 이상 넘어 갈 수 없는 곳에 도착하면 버튼이 투명화됩니다.
</td>
</tr>
</table>







